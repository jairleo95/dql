...
import gym
from res.dql_tf2.torch_deep_q_model import Agent
...
if __name__ == '__main__':
    ...
    for i in range(numGames):
        ...
        observation = env.reset()
        print("state.shape:" + str(observation.shape))
        #state.shape:(210, 160, 3)
        frames = [np.sum(observation[15:200,30:125], axis=2)]
        print("frames(new_obs):"+str(np.array(frames).shape))
        #frames(new_obs):(1, 185, 95)
        score = 0
        lastAction = 0
        while not done:
            if len(frames) == 3:
