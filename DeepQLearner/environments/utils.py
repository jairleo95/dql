# -*- coding: utf-8 -*-
"""
Created on Mon Sep  7 20:35:54 2020
...
class ResizeReshapeFrames(gym.ObservationWrapper):
    def __init__(self, environment):
        ...
        if len(self.observation_space.shape) == 3:
            ...
            self.observation_space = gym.spaces.Box(0, 255, (self.desired_channels, self.desired_height, self.desired_width), dtype = np.unit8)
            
    def observation(self, obs):
        if len(obs.shape) == 3:
            obs = cv2.resize(obs, (self.desired_width, self.desired_height))
            if obs.shape[2] < obs.shape[0]:
                obs = np.reshape(obs.shape[2], obs.shape[1], obs.shape[0])
